...
def dev_stpfl12_main(PART):
    ...
    FIT          = 7 if DET == 'mdtax4' else 8
    file_prefix  = PART[-55:-27]
    outfile_path = PART[:-61] + '/pass5/'
    logfile_path = PART[:-61] + '/log/pass5/'
    os.system('''
    dev_stpfl12_main -det {3} -db -o {2}/{1}.ps5.wsds.tlhbgeomnp.dst.gz -fit {4} {0} > {5}/{1}.log;
    '''.format(PART,file_prefix,outfile_path,DET_ID,FIT,logfile_path))
