...
def get_ontime(det,fd_path):
    ...
    for otf in ontime_list:
        if det == 'mdtax4':
            ...
            if otf[-42:-18] == '20191022/y2019m10d22p001':
                #print(otf)
                os.system('readlink -f {0} > {1}'.format(otf,filename))
            elif int(otf[-42:-34]) >= 20191022:
                #print(otf)
                os.system('readlink -f {0} >> {1}'.format(otf,filename))
            else:
                continue
...
if __name__=='__main__':
    ...
    filepath                 = '{0}/mc/processing/hAp.root'.format(fd_path) if not hyb else '{0}/hAp.root'.format(hyb_path)
    ...
    if not hyb:
        ...
        hExposure.SaveAs("{0}/hExposure.root".format(hyb_path))
        ROOT.gPad.SaveAs("{0}/plots/hExposure_20190625-{1}.png".format(hyb_path,last_date))
